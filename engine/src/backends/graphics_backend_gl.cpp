...
public:
	GLShader(const pxl::ShaderData& data) : _id(0)
	{
		...
		{
			const GLchar* src = (const GLchar*)data.vertex.data();
		...
		{
			const GLchar* src = (const GLchar*)data.pixel.data();
...
const pxl::ShaderData s_palette_shader = {
		"#version 330\n"
		"uniform mat4 u_matrix;\n"
		"layout(location=0) in vec2 a_position;\n"
		"layout(location=1) in vec2 a_tex;\n"
		...
		"{\n"
		"	vec4 color = texture(u_texture, v_tex);\n"
		"	vec4 palette_color = texture(u_palette, vec2(color.r, 0.5));"
		"	o_color = palette_color * v_col;\n"
		"}"
};
...
pxl::ShaderRef pxl::graphics::createPaletteShader() {
	return pxl::graphics::createShader(s_palette_shader);
}
